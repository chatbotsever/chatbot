...
/**
 * Get random picture of girl
 * @returns URL to image
 */
const getGirlData = (): string => {
  const img = randomIntFromInterval(1, MAX_CAT_IMG);
  return `https://hungs20.github.io/stores/${img}.jpg`;
};
 ...
 */
const sendGirlPic = async (id1: string): Promise<void> => {
  const url = getGirlData();

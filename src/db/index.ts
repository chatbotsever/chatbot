...
const initCache = async (): Promise<boolean> => {
  try {
    ...
    st.forEach(async (item) => {
      await cache.stateWrite(item.id, item.state);
    });
 ...
 */
const setState = async (id: string, state: StateEnum): Promise<void> => {
  await Promise.all([cache.stateWrite(id, state), mongo.stateWrite(id, state)]);
};

/**
 * Get state of user from database.
 * Return `null` if not available.
 * @param id - ID of user
 */
const getState = async (id: string): Promise<StateEnum | null> => {
  return await cache.stateFind(id);
};

...
const stateCacheMutex = new Mutex();
 ...
 * Save state in cache
 * @param id - ID of user
 * @param state - Gender of user
 */
const stateWrite = async (id: string, state: StateEnum): Promise<void> => {
  const release = await stateCacheMutex.acquire();
  try {
    stateCache.set(id, state);
  } catch (err) {
    logger.logError('cache::stateWrite', 'This should never happen', err, true);
  } finally {
    release();
  }
};
 ...
 * @param id - ID of user
 */
const stateFind = async (id: string): Promise<StateEnum | null> => {
  let ret: StateEnum | null = null;

  const release = await stateCacheMutex.acquire();
  try {
    ret = stateCache.has(id) ? stateCache.get(id) : null;
  } catch (err) {
    logger.logError('cache::stateFind', 'This should never happen', err, true);
  } finally {
    release();
  }
 ...
 * Return state data
 */
/*const stateRead = async (): Promise<GenderEntry[]> => {
  const ret: StateEntry[] = [];

  const release = await stateCacheMutex.acquire();
  try {
    let key = stateCache.nextKey();
    while (key) {
      const state: StateEnum = stateCache.get(key);
      ret.push({ id: key, state });
      key = chatRoomCache.nextKey(key);
    }
  } catch (err) {
    logger.logError('cache::stateRead', 'This should never happen', err, true);
  } finally {
    release();
  }
